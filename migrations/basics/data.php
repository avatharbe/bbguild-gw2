<?php
/**
 * bbGuild GW2 plugin - Data seeding migration
 *
 * Seeds Guild Wars 2 factions, classes, races, and roles
...
namespace avathar\bbguild_gw2\migrations\basics;
...
class data extends \phpbb\db\migration\container_aware_migration
{
	...
	public function effectively_installed()
	{
		...
		$sql = 'SELECT COUNT(*) AS cnt FROM ' . $games_table . " WHERE game_id = 'gw2'";
	...
	{
		$installer = $this->container->get('avathar.bbguild_gw2.installer');
		$installer->install($this->get_table_names(), 'gw2', 'Guild Wars 2', '', '', 'eu');
	}

	public function remove_game_data()
	{
		$installer = $this->container->get('avathar.bbguild_gw2.installer');
		$installer->uninstall($this->get_table_names(), 'gw2', 'Guild Wars 2');
	}
